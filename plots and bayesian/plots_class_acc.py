...
for i, param in enumerate(kfac_params):
    ...
    if i == 0:
        plt.plot(np.linspace(0, time_limit, len(losses["accuracy"])), losses["accuracy"], color="cornflowerblue", label = "KFAC")
    else:
        plt.plot(np.linspace(0, time_limit, len(losses["accuracy"])), losses["accuracy"], color="cornflowerblue")
...
for i, param in enumerate(first_params):
    ...
    losses["accuracy"].insert(0, first_loss)
    if i == 0:
        plt.plot(np.linspace(0, time_limit, len(losses["accuracy"])), losses["accuracy"], color="indianred", label = "SGD")
    else:
        plt.plot(np.linspace(0, time_limit, len(losses["accuracy"])), losses["accuracy"], color="indianred")

plt.legend()
plt.title("Classification accuracy over 10 seconds")
plt.xlabel("time in seconds")
plt.ylabel("accuracy")
plt.savefig('plots/class_accuracy_as_a_function_time_soft_max.png', bbox_inches='tight', dpi=200)
with open('plots/figures/class_accuracy_as_a_function_time_soft_max.pkl','wb') as file:
    pickle.dump(ax, file)
...
for i, param in enumerate(kfac_params):
    ...
    losses, t, net = kfac_optimizer(net, loss_func, problem_type, num_epochs, lr, num_layers, d, batch_size,
                                    ...
                                    train_device, run_test, verbose, time_limit, update_limit)
    if i == 0:
        plt.plot(np.linspace(0, update_limit, len(losses["accuracy"])), losses["accuracy"], color="cornflowerblue", label = "KFAC")
    else:
        plt.plot(np.linspace(0, update_limit, len(losses["accuracy"])), losses["accuracy"], color="cornflowerblue")
...
for i, param in enumerate(first_params):
    ...
    losses["accuracy"].insert(0, first_loss)
    if i == 0:
        plt.plot(np.linspace(0, update_limit, len(losses["accuracy"])), losses["accuracy"], color="indianred", label = "SGD")
    else:
        plt.plot(np.linspace(0, update_limit, len(losses["accuracy"])), losses["accuracy"], color="indianred")

plt.legend()
plt.title("Classification accuracy over 50 model updates")
plt.xlabel("model updates")
plt.ylabel("test accuracy")
plt.savefig('plots/class_accuracy_as_a_function_updates.png', bbox_inches='tight', dpi=200)
with open('plots/figures/class_accuracy_as_a_function_updates.pkl','wb') as file:
    pickle.dump(ax, file)
# with open('plots/batch_size_fixed_updates.pkl', 'rb') as file:
#     ax = pickle.load(file)
#plt.show()
