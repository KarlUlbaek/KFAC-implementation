...
def baysian_black_box(to_optimize, net, problem_type, num_epochs, d, momen_conv, scaling_1, scaling_2, train_device,
    ...
    for i in range(repetitions):
        ...
        losses += min(loss["train"]) # append losses
...
mixed_domain =[
                {'name': 'lr', 'type': 'continuous', 'domain': (0.01, 1)},
                {'name': 'batch_size', 'type': 'continuous', 'domain': (500, 20000)},
...
while i < 3:
    ...
    try:
        ...
        df.to_csv("kfac_reg_v1.csv", index=False)
